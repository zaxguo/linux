...
static void *replay(void *args) {
	...
	total = 0;
	idx = 0;
	ret = snprintf(dest, 50, "/sybil/fs0/%d.pub", arg->tid);
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < 1; i++) {
		int j;
		char key_file[128];
		for (j = 0; j < 20 * fs_cnt; j++) {
			int ret;
			ret = snprintf(key_file, 50, "/sybil/fs%d/%d.pub", i, j);
			ret = open(key_file, O_RDWR | O_CREAT);
	...
	i = open("/proc/sys/vm/drop_caches", O_RDWR);
	if (i > 0) {
		ret = write(i, "3\n", 2);
		if (ret != 2) {
			printf("fail to clear caches!\n");
		}
		printf("clearing all caches!\n");
	} else {
