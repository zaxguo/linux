...
static void *test_write(void *args) {
	...
	printf("[%d] has written %d bytes to %s. cnt = %d\n", arg->tid, total, dest, idx);
}
...
static int* construct_lib(char *lib_path) {
	...
	while (((nread = getline(&line, &len, lib)) != -1) && records < MAX_RECORDS) {
		int cnt, idx, res;
		cnt = 0; idx = 0;
		...
		rosbag_lib[records++] = res;
	}
	free(line);
	fclose(lib);
	printf("constructing %d records...\n", records);
	return rosbag_lib;
}
